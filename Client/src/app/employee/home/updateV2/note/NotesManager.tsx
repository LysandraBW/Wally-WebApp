        ...
        <div className="row-start-5 row-span-1 col-start-1 col-span-1 relative flex flex-col grow h-min">
            <div className="gap-4 bg-white relative after:absolute after:w-[1px] after:h-full after:top-0 after:left-[0px] after:bg-gray-300 before:absolute before:w-[1px] before:h-full after:top-0 before:right-[0px] before:bg-gray-300 h-full">
                <table className="w-full">
                    <tbody>
                        <tr>
                            <td className="w-0 p-0 text--center bg-white font-medium px-4 text-03 tracking-wide whitespace-nowrap border border-gray-300 align-top pt-2">{props.notesManager.item.itemName}</td>
                            <td className="p-0 border border-gray-300">
                                <div className="relative bg-gray-50 flex flex-col gap-0 py-6 px-4  border-b border-b-gray-200">
                                    ...
                                    <CreateItemButton
                                        onCreate={props.notesManager.handleCreateItem}
                                    />
                                </div>
                                <div className="px-4 py-4">
                                    <NoteDisplay
                                        items={props.notesManager.newItems}
                                        onUpdate={props.notesManager.handleUpdateItem}
                                        onDelete={(ID: string) => {
                                            props.notesManager.deleteItem(ID);
            ...
            <SaveResetButtons
                onSave={() => {
                    props.notesManager.saveUpdates();
                    // updateManagerContext.setChangesMade(props.tab, false);
                }}
                onReset={() => {
                    props.notesManager.resetUpdates();
                    // updateManagerContext.setChangesMade(props.tab, false);
                }}
            />
        </div>   
