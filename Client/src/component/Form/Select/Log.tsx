    ...
    return (
        <div>
            {props.values.length !== 0 &&
                <ul className="flex gap-1 overflow-x-scroll scroll-hide">
                    {props.values.map((value, i) => (
                        <li
                            key={i}
                            onClick={(e) => deleteValue(e, value)}
                            className="field flex justify-between items-center gap-2 py-0.5 pr-1 pl-2 bg-gray-100 whitespace-nowrap hover:bg-gray-200 hover:border-gray-300"
                        >
                            <span className="text-01 font-medium">{props.valueToLabel[value]}</span>
                            <CrossIcon
                                top="0.05px"
                                width="13"
                                height="13"
