...
export async function execTDeleteAppointments(data) {
    try {
...
export const testTDeleteAppointments = z.object({
    sessionID: isUniqueIdentifier,
    appointmentIDs: isCommaJoinedUUIDArray
});

export const tDeleteAppointments = {
    test: (input) => testTDeleteAppointments.safeParse(input),
    exec: (input) => execTDeleteAppointments(input)
}