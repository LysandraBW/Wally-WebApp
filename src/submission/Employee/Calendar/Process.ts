...
export async function processEventsForm(
    ...
    toUpdateEventIDs.forEach(async (eventID) => {
        ...
        if (!sameObject(refEvent, curEvent, ['Name', 'Summary', 'Date'])) {
            processedEventsForm.Update.push({ 
                ...
                Date: updatedValue(toDatabaseDateTime(refEvent.UpdatedDate), toDatabaseDateTime(curEvent.UpdatedDate))
            });
        }
    ...
    toInsertEventIDs.forEach((eventID) => {
        ...
        processedEventsForm.Insert.Event.push({
            ...
            Date: toDatabaseDateTime(curEvent.UpdatedDate),
